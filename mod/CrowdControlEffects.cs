...
namespace Oxide.Plugins
{
    ...
    public class CrowdControlEffects : RustPlugin
    {
        ...
        private void RegisterBuiltInEffects()
        {
            ...
            if (effects.Count == 0)
            {
                PrintWarning("No built-in Crowd Control effects were loaded from effects.json.");
                return;
            }
        ...
        private JArray LoadBuiltInEffects()
        {
            ...
            if (!File.Exists(path))
            {
                PrintWarning("effects.json not found; cannot register built-in Crowd Control effects.");
                return results;
            }
            ...
            try
            {
                ...
                {
                    var effectId = prop.Name;
                    if (!IsSupportedBuiltInEffectId(effectId))
                    {
                        continue;
                    }
                    ...
                    {
                        ["effectID"] = effectId,
                        ["name"] = effectObj?.Value<string>("name") ?? effectId,
                        ["description"] = effectObj?.Value<string>("description") ?? "Built-in Rust effect.",
                        ["price"] = Math.Max(0, effectObj?.Value<int?>("price") ?? 0),
                        ["syncMenu"] = false
                    };

                    var duration = effectObj?["duration"] as JObject;
                    if (duration != null)
                    {
                        registerObj["duration"] = (JObject)duration.DeepClone();
                    }
            ...
            {
                PrintWarning($"Failed to load built-in Crowd Control effects: {ex.Message}");
            }
